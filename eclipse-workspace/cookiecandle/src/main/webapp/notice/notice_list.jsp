...
<%
	...
	int pageNum=1;
	if(request.getParameter("pageNum")!=null){//전달값 있는 경우
	...
	List<NoticeDTO> noticeList = NoticeDAO.getDAO().selectAllNoticeList(startRow,endRow,search,keyword);
	...
	<table class="table table-hover">
  		...
  		<tbody>
  			<% for(NoticeDTO notice:noticeList){%>
			    <tr>
			    	<td><%=notice.getnNo() %></td>
				    <td><a href="index.jsp?workgroup=notice&work=notice_view&num=<%=notice.getnNo()%>"><%=notice.getnTitle() %></a></td>
				    <td><%=notice.getnView() %></td>
	...
	<div class="notice_foot">
		<div class="pagination">
			<% if(startPage>blockSize) {//첫번째 블럭이 아닌 경우 - 링크 설정 %>
				<a href="index.jsp?workgroup=notice&work=notice_list&pageNum=1&search=<%=search%>&keyword=<%=keyword%>"><<</a>
				<a href="index.jsp?workgroup=notice&work=notice_list&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>"><</a>
			<% } else { %>
				<span><<</span>
				<span><</span>
			<% } %>
			<% for(int i=startPage;i<=endPage;i++) { %>
				<% if(pageNum!=i) {//요청 페이지 번호와 이벤트 페이지 번호가 다른 경우 - 링크 설정 %>
					<a href="index.jsp?workgroup=notice&work=notice_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>"><span class="numberPage"><%=i %></span></a>
			...
			<% if(endPage!=totalPage) {//마지막 블럭이 아닌 경우 - 링크 설정 %>
				<a href="index.jsp?workgroup=notice&work=notice_list&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">></a>
				<a href="index.jsp?workgroup=notice&work=notice_list&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">>></a>
			<% } else { %>
				<span>></span>
				<span>>></span>
			<% } %>
		...
		<div class="board_list_btn">
			<% if(userLogin != null && userLogin.getStatus() == 9) {//로그인 사용자 중 게시글 작성자이거나 관리자인 경우 %>	
				<a class="btn btn-outline-dark" href="index.jsp?workgroup=notice&work=notice_write">write</a>
			<% } %>
		</div>
	</div>
	<br>
	...
	<div class="notice_search">
		<form action="index.jsp?workgroup=notice&work=notice_list" method="post">
			...
			<select id="search" name="search">
				<option value="n_title" selected="selected">&nbsp;제목&nbsp;</option>
